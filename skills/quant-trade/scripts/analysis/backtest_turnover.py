...
def backtest(
    ...
    for current_date in progress:
        ...
        # 候选列表：(symbol, turnover_5d_avg)
        candidates_with_turnover = []
        for sym in active_pool:
            ...
            if all(rps_scores.get(f'{p}d_rps', 0) >= rps_threshold for p in rps_periods):
                df = stock_data[sym]
                df_cut = df[df.index <= current_date]
                if len(df_cut) < 5:   # 需要至少5天数据计算成交额
                    continue
                # 计算5日平均成交额（美元）
                turnover = (df_cut['close'] * df_cut['volume']).rolling(5).mean().iloc[-1]
                if pd.isna(turnover) or turnover <= 0:
                    continue
                candidates_with_turnover.append((sym, turnover))

        # 按成交额降序排序
        candidates_with_turnover.sort(key=lambda x: x[1], reverse=True)
        candidate_symbols = [sym for sym, _ in candidates_with_turnover]
